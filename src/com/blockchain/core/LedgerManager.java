...
public class LedgerManager {
	...
	/**
	 * Constructor for LedgerManager. All that is needed is the path to the address
	 * database file.
	 * 
	 * All peers on the network should have an identical copy of the LedgerManager
	 * object at any given time. Due to latency and whatnot, that doesn't happen,
	 * but a fully synchronized network would have the same ledger on every node at
	 * any time.
	 * 
	 * @param addressDatabase
	 *            The String representation of the address database file
	 */
	...
	public String getLedgerHash() {
		...
		for (int i = 0; i < addresses.size(); i++) {
			ledger += addresses.get(i) + ":" + addressBalances.get(addresses.get(i)) + ":"
					+ addressSignatureCounts.get(addresses.get(i)) + "\n";
		}
		try {
	 ...
	 * 
	 * @param lastBlockNum
	 *            The latest block applied to this tree
	 */
	...
	public boolean executeTransaction(String transaction) {
		try {
			...
			if (!merkleAddressUtility.verifyMerkleSignature(transactionMessage, signatureData, sourceAddress,
					signatureIndex)) {
				return false; // Signature does not sign transaction message!
			}
			if (getAddressSignatureCount(sourceAddress) + 1 != signatureIndex) {
				return false; // The signature is valid, however it isn't using the expected signatureIndex.
								// Blocked to ensure a compromised Lamport key from a previous transaction can't
								// be used.
			}
			...
			for (int i = 2; i < transactionParts.length - 2; i += 2) // -2 because last two parts of transaction are the
																		// signature and signature index
			{
			...
			for (int i = 0; i < destinationAddresses.size(); i++) {
				addressBalances.put(destinationAddresses.get(i),
						getAddressBalance(destinationAddresses.get(i)) + destinationAmounts.get(i));
			}
	...
	/**
	 * This method reverse-executes a given transaction String of the format
	 * InputAddress;InputAmount;OutputAddress1;OutputAmount1;OutputAddress2;OutputAmount2...;SignatureData;SignatureIndex
	 * Used primarily when a blockchain fork is resolved, and transactions have to
	 * be reversed that existed in the now-forked block(s).
	 * 
	 * @param transaction
	 *            String-formatted transaction to execute
	 * 
	...
	public boolean reverseTransaction(String transaction) {
		try {
			...
			if (getAddressSignatureCount(sourceAddress) + 1 != signatureIndex) {
				...
				// return false; //The signature is valid, however it isn't using the expected
				// signatureIndex. Blocked to ensure a compromised Lamport key from a previous
			...
			for (int i = 0; i < destinationAmounts.size(); i++) {
				if (getAddressBalance(destinationAddresses.get(i)) < destinationAmounts.get(i)) {
					System.out.println("[CRITICAL ERROR] ADDRESS " + destinationAddresses.get(i) + " needs to return "
							+ destinationAmounts.get(i) + " but only has "
							+ getAddressBalance(destinationAddresses.get(i))); // BIG PROBLEM THIS SHOULD NEVER HAPPEN
	...
	public boolean writeToFile() {
		try {
			...
			for (int i = 0; i < addresses.size(); i++) {
				out.println(addresses.get(i) + ":" + addressBalances.get(addresses.get(i)) + ":"
						+ addressSignatureCounts.get(addresses.get(i)));
			}
			out.close();
		} catch (Exception e) {
	 ...
	 * 
	 * @param address
	 *            Address to retrieve the latest index for
	 * 
	 ...
	 *            Address to adjust
	 * @param adjustment
	 *            Amount to adjust address's signature count by. This can be
	 *            negative.
	 * 
	...
	/**
	 * Adjusts the balance of an address by a given adjustment, which can be
	 * positive or negative.
	 * 
	 * @param address
	 *            Address to adjust the balance of
	 * @param adjustment
	 *            Amount to adjust account balance by
	 * 
	 * @return boolean Whether the adjustment was successful
	 */
	 ...
	 * @param newAmount
	 *            New amount to set as the balance of address
	 * 
