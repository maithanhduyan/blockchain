...
/**
 * TransactionUtility simplifies a few basic tasks dealing with transaction
 * parsing and verification.
 */
public class TransactionUtility {
	/**
	 * Tests whether a transaction is valid. Doesn't test account balances, but
	 * tests formatting and signature verification.
	 * 
	 * @param transaction
	 *            Transaction String to test
	 * 
	...
	public static boolean isTransactionValid(String transaction) {
		...
		try {
			...
			for (int j = 3; j < transactionParts.length - 2; j += 2) // Element 3 (4th element) and each subsequent
																		// odd-numbered index up to transactionParts
																		// should be an output amount.
			{
	...
	/**
	 * Transactions on the Curecoin 2.0 network from the same address must occur in
	 * a certain order, dictated by the signature index. As such, We want to order
	 * all transactions from the same address in order. The order of transactions
	 * from different addresses does not matter--coins will not be received and
	 * spent in the same transaction.
	 * 
	 * @param transactionsToSort
	 *            ArrayList<String> containing String representations of all the
	 *            addresses to sort
	 * 
	 * @return ArrayList<String> All of the transactions sorted in order for block
	 *         inclusion, with any self-invalidating transactions removed.
	 */
		...
		for (int i = 0; i < transactionsToSort.size(); i++) {
			...
			if (sortedTransactions.size() == 0) {
				...
				long index = Long.parseLong(
						transactionsToSort.get(i).split(";")[transactionsToSort.get(i).split(";").length - 1]);
				boolean added = false;
	...
	/**
	 * Signs a Transaction built with the provided sending address and amount, and
	 * destination address(es) and amount(s).
	 * 
	 * @param privateKey
	 *            The private key for inputAddress
	 * @param inputAddress
	 ...
	 * @param outputAmounts
	 *            Amounts lined up with addresses to send
	 * @param signatureIndex
	 *            The signature index to use
	 * 
	 * @return String The full transaction, formatted for use in the Curecoin 2.0
	 *         network, including the signature and signature index. Returns null if
	 *         transaction is incorrect for any reason.
	 */
	public static String signTransaction(String privateKey, String inputAddress, long inputAmount,
			ArrayList<String> outputAddresses, ArrayList<Long> outputAmounts, long index) {
		if (inputAddress == null || outputAddresses == null || inputAmount <= 0) // Immediate red flags
		{
			return null;
		}
		if (outputAddresses.size() != outputAmounts.size()) // Output addresses and amounts go together, and so each
															// ArrayList must be the same size
		{
			return null;
		}
		String fullTransaction = inputAddress + ";" + inputAmount; // The start of the Transaction
		for (int i = 0; i < outputAddresses.size(); i++) // Didn't bother doing address checks here, as they will be
															// conducted in isTransactionValid()
		{
			fullTransaction += ";" + outputAddresses.get(i) + ";" + outputAmounts.get(i);
		}
		fullTransaction += ";"
				+ new MerkleAddressUtility().getMerkleSignature(fullTransaction, privateKey, index, inputAddress) + ";"
				+ index; // Now it's actually the 'full transaction'
