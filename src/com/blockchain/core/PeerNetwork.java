...
/**
 * This thread listens on a provided port (8015 by default) for incoming
 * connections, and attempts to make connections to external peers based on
 * guidance from MainClass. It needs a bit of help with memory management and
 * resource deallocation, but otherwise it works. Good enough for 2.0.0a1.
 * 
...
public class PeerNetwork extends Thread {
	...
	public void run() {
		try {
			...
			while (shouldRun) // Doesn't actually quit right when shouldRun is changed, as while loop is
								// pending.
			{
	...
	/**
	 * Announces the same message to all peers simultaneously. Useful when
	 * re-broadcasting messages.
	 * 
	 * @param toBroadcast
	 *            String to broadcast to peers
	 */
	...
	/**
	 * Announces the same message to all peers except the ignored one
	 * simultaneously. Useful when re-broadcasting messages. Peer ignored as it's
	 * the peer that sent you info.
	 * 
	 * @param toBroadcast
	 *            String to broadcast to peers
	 * @param peerToIgnore
	 *            Peer to not send broadcast too--usually the peer who sent
	 *            information that is being rebroadcast
	 */
